...
export const GameTaskExerciseListForTeacher = ({taskIndex, onExerciseSelected}: TextTaskExerciseListForTeacherProps) => {
  const { t } = useTranslation();
  const { getExerciseIndexByTaskIndexAndExerciseType, lesson } = useLessonForm();
  
  const taskTypeTitle = t(`pages.lesson.form.tasks.taskType.${TaskType.GameTask}`);
