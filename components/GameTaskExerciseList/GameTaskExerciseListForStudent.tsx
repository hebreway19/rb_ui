...
import { GameTaskExerciseTypes, TaskType, ViewModes } from "../../constants";
import {useTranslation} from "next-i18next";
import {Col, Row, Space} from "antd";
import { Exercise } from "../../types";
import {HebPanel, HebTooltip, HebTypography, HebDivider} from "../HebElements";
...
export const GameTaskExerciseListForStudent = ({exercises, mode = ViewModes.DEFAULT, onExerciseSelected}: GameTaskExerciseListForStudentProps) => {
  ...
  const currentExercises = exercises.filter(({__t}) => Object.values(GameTaskExerciseTypes).includes(__t as GameTaskExerciseTypes));
  
  const taskTypeLabel: string = t(`pages.lesson.form.tasks.taskType.${TaskType.GameTask}`) + ". " + t("entities.exercise.exercises");
  ...
  return (
    <Space direction="vertical"
      ...
      <Row>
        <Col xs={24}>
          <HebDivider>{taskTypeLabel}</HebDivider>
        </Col>
        <Col xs={24}>
          <Row justify="start"
